...
BEGIN
	if inserting or updating then
		if :new.diffusionEn3D != 0 then
			...
			if salleEn3D != :new.diffusionEn3D then 
				raise salle_compatible3D;
			end if;
		end if;
	end if;
EXCEPTION 
	when  salle_compatible3D then
		raise_application_error(-20007,'la salle doit etre compatible avec 3D');
END;
/